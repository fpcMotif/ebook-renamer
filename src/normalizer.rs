...
pub fn normalize_files(mut files: Vec<FileInfo>) -> Result<Vec<FileInfo>> {
    for file_info in &mut files {
        ...
        debug!("Normalized: {} -> {}", file_info.original_name, new_name);
    }
...
fn parse_filename(filename: &str, extension: &str) -> Result<ParsedMetadata> {
    ...
    base = Regex::new(r"[-\s]*\(\d{1,2}\)\s*$")
        .unwrap()
        .replace(&base, "")
        .to_string(); // (1), (2) at end
    base = Regex::new(r"-\d{1,2}\s*$")
        .unwrap()
        .replace(&base, "")
        .to_string(); // -2, -3 at end
    base = Regex::new(r"-\d{1,2}\s+\(")
        .unwrap()
        .replace(&base, " (")
        .to_string(); // -2 before (year)
...
fn clean_noise_sources(s: &str) -> String {
    ...
    result = strip_trailing_noise_tokens(&result);

    result.trim().to_string()
}

fn strip_trailing_noise_tokens(s: &str) -> String {
    let mut result = s.trim_end().to_string();
    loop {
        let mut changed = false;
        let trimmed = result.trim_end_matches(|c: char| c == '.' || c.is_whitespace());

        if let Some(new_len) = strip_enclosed_suffix(trimmed, '(', ')') {
            result.truncate(new_len);
            changed = true;
        } else if let Some(new_len) = strip_enclosed_suffix(trimmed, '[', ']') {
            result.truncate(new_len);
            changed = true;
        } else if let Some(new_len) = strip_dash_suffix(trimmed) {
            result.truncate(new_len);
            changed = true;
        } else if let Some(new_len) = strip_plain_suffix(trimmed) {
            result.truncate(new_len);
            changed = true;
        }
...
fn strip_enclosed_suffix(s: &str, open: char, close: char) -> Option<usize> {
    if !s.ends_with(close) {
        return None;
    }
    let without_close = &s[..s.len() - close.len_utf8()];
    if let Some(open_idx) = without_close.rfind(open) {
        let inner = without_close[open_idx + open.len_utf8()..].trim();
        if is_trailing_noise_token(inner) {
...
fn strip_dash_suffix(s: &str) -> Option<usize> {
    for delimiter in ['-', '–', '—', '_'] {
        if let Some(pos) = s.rfind(delimiter) {
            let suffix = s[pos + delimiter.len_utf8()..].trim();
            if is_trailing_noise_token(suffix) {
                return Some(pos);
            }
        }
    }
...
fn strip_plain_suffix(s: &str) -> Option<usize> {
    if s.is_empty() {
        return None;
    }
    if let Some((idx, ch)) = s.char_indices().rev().find(|(_, ch)| ch.is_whitespace()) {
        let start = idx + ch.len_utf8();
        let candidate = s[start..].trim();
        if is_trailing_noise_token(candidate) {
            return Some(idx);
        }
    }
    if is_trailing_noise_token(s) {
        return Some(0);
    }
    None
}

fn is_trailing_noise_token(value: &str) -> bool {
    if value.is_empty() {
        return false;
    }
    ...
    TOKENS.iter().any(|token| lowered == *token)
}
...
fn smart_parse_author_title(s: &str) -> (Option<String>, String) {
    ...
    if let Some(caps) = re_trailing_author.captures(s) {
        ...
        if is_likely_author(author_part)
            && !is_publisher_or_series_info(&format!("({})", author_part))
        {
            return (
    ...
    let re_multi_author =
        Regex::new(r"^([A-Z][^:]+?),\s*([A-Z][^:]+?)\s*(?:--|[-:–—])\s+(.+)$").unwrap();
    if let Some(caps) = re_multi_author.captures(s) {
        ...
        if is_likely_author(author1) && is_likely_author(author2) {
            let authors = format!(
                "{}, {}",
                clean_author_name(author1),
                clean_author_name(author2)
            );
            return (Some(authors), clean_title(title_part));
        }
    }
...
fn is_likely_author(s: &str) -> bool {
    ...
    let has_non_latin = s.chars().any(|c| c.is_alphabetic() && !c.is_ascii());
...
mod tests {
    ...
    fn test_parse_with_series_prefix() {
        let metadata = parse_filename("B. R. Tennison - Sheaf Theory (1976).pdf", ".pdf").unwrap();
    ...
    fn test_parse_author_before_title_with_publisher() {
        ...
        assert_eq!(
            metadata.authors,
            Some("Ernst Kunz, Richard G. Belshoff".to_string())
        );
    ...
    fn test_multi_author_with_commas() {
        ...
        assert_eq!(
            metadata.authors,
            Some("Thomas H. Wolff, Izabella Aba, Carol Shubin".to_string())
        );
    ...
    fn test_wavelets_with_multiple_authors_and_z_library() {
        ...
        assert_eq!(
            metadata.authors,
            Some("Michel Misiti, Yves Misiti, Georges Oppenheim etc.".to_string())
        );
    ...
    fn test_mani_mehra_wavelets() {
        ...
        assert_eq!(
            metadata.title,
            "Wavelets Theory and Its Applications A First Course"
        );
        assert!(!metadata.title.contains("Z-Library"));
    }
    ...
    fn test_en_dash_and_copy_noise() {
        let metadata =
            parse_filename("Smith & Wesson – Firearms Copy (Z-Library).pdf", ".pdf").unwrap();
    ...
    fn test_year_scan_parenthetical() {
        let metadata = parse_filename("History 1917 (2021 Scan).pdf", ".pdf").unwrap();
    ...
    fn test_truncated_author_parenthesis() {
        let metadata = parse_filename("Quantum Fields (Incomplete Author.pdf", ".pdf").unwrap();
