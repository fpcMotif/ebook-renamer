...
fn is_publisher_or_series_info(s: &str) -> bool {
    ...
    let version_regex = Regex::new(r"\b(v|ver|version)\.?\s*\d+(\.\d+)*\b").unwrap();
    if version_regex.is_match(&s_lower) {
        return true;
    }
    
    // Check for page count patterns: 500 pages, 500pp, 500 P
    let page_regex = Regex::new(r"\b\d+\s*(?:pages?|pp?\.?|P)\b").unwrap();
    if page_regex.is_match(&s_lower) {
        return true;
    }
    
    // Check for language tags: (English), (中文), English Edition
    let lang_patterns = [
