...
fn extract_series_info(s: &str) -> (Option<String>, String) {
    ...
    // Pattern 0: Abbreviated series prefix (e.g., "GTM 52 - Author - Title")
    for (_series_name, abbr) in &series_mappings {
        let pattern = format!(r"^{}\s*(\d+)\s*[-\s]", regex::escape(abbr));
    ...
    if let Some(caps) = re_paren_series.captures(&result) {
        ...
        for (series_name, abbr) in &series_mappings {
            if series_part
                .to_lowercase()
                .contains(&series_name.to_lowercase())
            {
...
mod tests {
    ...
    #[test]
    fn test_parse_with_abbreviated_series_prefix() {
        let metadata = parse_filename(
            "GTM 52 - Saunders Mac Lane - Categories for the Working Mathematician.pdf",
            ".pdf",
        )
        .unwrap();
    ...
    fn test_edition_detection_2nd() {
        let metadata =
            parse_filename("James Munkres - Topology - 2nd Edition (2000).pdf", ".pdf").unwrap();
    ...
    fn test_comprehensive_all_metadata() {
        ...
        assert_eq!(
            new_name,
            "Author Name - Book Title Vol 3 [CSAM 100] (2020, 2nd ed).pdf"
        );
    }
}
