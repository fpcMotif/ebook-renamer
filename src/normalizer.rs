...
fn clean_structured_noise(s: &str) -> String {
    ...
    result = remove_duplicate_markers(&result);
...
fn is_likely_author(s: &str) -> bool {
    ...
    if non_author_keywords.iter().any(|k| s_lower.contains(k)) {
        return false;
    }
...
fn is_publisher_or_series_info(s: &str) -> bool {
    ...
    if FORMAT_KEYWORDS
        .iter()
        .any(|kw| lower.contains(kw) || trimmed.contains(kw))
    {
        return true;
    }
    ...
    let non_letter_count = trimmed
        .chars()
        .filter(|c| !c.is_alphabetic() && *c != ' ')
        .count();
    let has_volume_hint = lower.contains("vol")
...
mod tests {
    ...
    fn test_pdfdrive_marker_removed() {
        let metadata = parse_filename("Jane Doe - Example Title - pdfdrive.pdf", ".pdf").unwrap();
        assert_eq!(metadata.authors, Some("Jane Doe".to_string()));
    ...
    fn test_copy_suffix_removed() {
        let metadata = parse_filename("John Smith - Sample Title Copy (1).pdf", ".pdf").unwrap();
        assert_eq!(metadata.authors, Some("John Smith".to_string()));
    ...
    fn test_unmatched_parenthesis_is_cleaned() {
        let metadata = parse_filename("Great Book (Jane Doe.pdf", ".pdf").unwrap();
